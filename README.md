...
**Because not every random vacation clip needs 60 fps.** If your partner's iPhone is once again set to 60 fps for a perfectly ordinary walk to the beach, tick **Limit video to a maximum of 30 FPS**. VMP caps only clips above 30 fps, leaves 24/25/30‑fps footage alone, and folds the frame‑rate reduction into the same FFmpeg pass as HEVC encoding and optional Full‑HD downscaling — no second quality hit.
...
- 📍 **Missing GPS repair** — proposes locations from nearby timed GPS anchors and stationary clusters, with batch review and precise manual pin placement.
